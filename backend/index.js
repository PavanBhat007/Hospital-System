...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const userResult = await pool.query(
      "INSERT INTO users (address, role, password) VALUES ($1, $2, $3) RETURNING id",
      [address, role, hashedPassword]
    );
    const userId = userResult.rows[0].id;

    if (role === "patient") {
      await contract.methods
        .addPatient(address, name, age, gender, bloodGroup)
        .send({ from: process.env.CONTRACT_OWNER_ADDRESS });
    } else if (role === "doctor") {
      await pool.query(
        "INSERT INTO doctors (user_id, name, age, gender, specialty, availability) VALUES ($1, $2, $3, $4, $5, $6)",
        [userId, name, age, gender, specialty, availability]
      );
      await contract.methods
        .addDoctor(address, name, age, gender, specialty, availability)
        .send({ from: process.env.CONTRACT_OWNER_ADDRESS });
    }
...
app.post("/patient/updateProfile", authenticateToken, async (req, res) => {
  ...
  try {
    await contract.methods
      .addPatient(req.user.address, name, age, gender, bloodGroup)
      .send({ from: process.env.CONTRACT_OWNER_ADDRESS });
...
app.post("/patient/bookAppointment", authenticateToken, async (req, res) => {
  ...
  try {
    await contract.methods
      .addAppointment(req.user.address, doctorAddress, details)
      .send({ from: process.env.CONTRACT_OWNER_ADDRESS });
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      await contract.methods
        .removeAppointment(req.user.address, doctorAddress, index)
        .send({ from: process.env.CONTRACT_OWNER_ADDRESS });
...
app.post("/doctor/updateProfile", authenticateToken, async (req, res) => {
  ...
  try {
    await contract.methods
      .addDoctor(req.user.address, name, age, gender, specialty, availability)
      .send({ from: process.env.CONTRACT_OWNER_ADDRESS });
    res.status(200).send("Profile updated");
  } catch (error) {
    res.status(500).send("Error updating profile");
